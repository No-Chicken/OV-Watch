...
{
	TimeSetMessage.nowdate.Year = (str[13]-'0')*10+str[14]-'0';
	TimeSetMessage.nowdate.Month = (str[16]-'0')*10+str[17]-'0';
	TimeSetMessage.nowdate.Date = (str[19]-'0')*10+str[20]-'0';
	TimeSetMessage.nowtime.Hours = (str[22]-'0')*10+str[23]-'0';
	TimeSetMessage.nowtime.Minutes = (str[25]-'0')*10+str[26]-'0';
	TimeSetMessage.nowtime.Seconds = (str[28]-'0')*10+str[29]-'0';
...
void MessageSendTask(void *argument)
{
	while(1)
	{
		if(HardInt_uart_flag)
		{
			...
			//set time//OV+SETTIME=2023-03-29.12:32:39
			else if(strlen(HardInt_receive_str)==30)
			{
				uint8_t cmd[30];
				StrCMD_Get(HardInt_receive_str,cmd);
				if(user_APPSy_EN && !strcmp(cmd,"OV+SETTIME"))
				{
